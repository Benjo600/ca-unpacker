...
import { probeIsAdmin } from "../lib/admin";
    ...
    <div className="flex min-h-dvh bg-paper-bright">
      <aside className="flex w-64 shrink-0 flex-col border-r border-rule/80 bg-desk text-desk-ink">
        <div className="border-b border-white/8 px-6 py-6">
          <Link to="/app" className="font-display text-xl tracking-tight text-paper no-underline">
            CA Unpacker
          </Link>
        ...
        <nav className="flex-1 space-y-1 p-3">
          ...
          >
            <LayoutDashboard className="h-4 w-4 shrink-0 opacity-80" strokeWidth={2} />
            Usage
          </NavLink>
          {isAdmin ? (
            <Link
              to="/admin"
              className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-desk-ink no-underline transition hover:bg-white/6 hover:text-paper"
            >
              <Shield className="h-4 w-4 shrink-0 opacity-80" strokeWidth={2} />
              Operator console
            </Link>
          ) : null}
        </nav>
